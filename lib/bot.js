...
function start() {
        ...
        Dota2.on("chatMessage", function(channel, personaName, message, chatData) {
            ...
            if (config.debug) {
                ...
                if (respond) {
                    ...
                    handlers.onGuildMessage(channel, steam64id, personaName, message, function (err, responseMsg) {
                        if (err == errCodes.SUCCESS) {
                            Dota2.sendMessage(channel, responseMsg);
                        }
                    });
            }
        });
    ...
    steamClient.on('friendMsg', function (source, message, type, chatter) {
        ...
        handlers.onPrivateMessage(source, personaName, message, function (err, responseMsg) {
            if (err == errCodes.SUCCESS) {
