 ...
 *         players: [{ObjectId("guid"), 'Radiant'}, {ObjectId("guid"), 'Dire'}], scores: [{ObjectId("guid"), 'Radiant'}],
...
DbClient.prototype.ReportWin = function ReportWin(db, playerSteam64Id, callback) {
        ...
        if (err) {
            ...
            FindActiveGameForPlayer(db, user._id, function (err, game) {
                if (err == errCodes.GAME_NOT_FOUND) {
                    ...
                    // report win by creating a score record
                    db.collection("games").update({ 'gameNum': game.gameNum, status: 'InProgress' }, { $addToSet: { scores: {} } }, function (err, docs) {
...
function GetPlayersForGame(db, players, callback) {
    db.collection("players").find({ _id: { $in: players } }).toArray(function (err, docs) {
        if (err) {
            return callback(errCodes.DATABASE_FAILURE);
        } else {
            callback(err, docs);
...
function IsPlayerInGame(gameObject, playerObjectId) {
    for (var i = 0; i < gameObject.players.length; i++) {
        if (gameObject.players[i].equals(playerObjectId))
            return true;
    }
    return false;
}
                ...
                FindUser(db, playerSteam64Id, function (err, user) {
                    if (err) {
                        ...
                        if (user.admin) {
                            ...
                            CancelGameInternal(db, gameNum, function (err) {
                                return callback(err);
                            });
                        } else {
 ...
 */
function CancelGameInternal(db, gameNum, callback) {
    FindLatestSeason(db, function (err, latestSeason) {
        db.collection("games").update({ seasonId: latestSeason._id, 'gameNum': gameNum, status: { $in: ['WaitingForPlayers', 'InProgress'] } }, { $set: { status: 'Cancelled' } }, function (err, doc) {
            if (callback) {
                if (err) {
                    return callback(errCodes.DATABASE_FAILURE);
                } else {
...
function FindActiveGameForPlayer(db, playerObjectId, callback) {
    FindLatestSeason(db, function (err, latestSeason) {
        db.collection("games").find({ $and: [{seasonId : latestSeason._id}, { status: { $in: ["WaitingForPlayers", "InProgress"] } },{ 'players': { $elemMatch: { $in: [playerObjectId] } } }] }).toArray(function (err, docs) {
            if (err) {
                return callback(errCodes.DATABASE_FAILURE);
            } else if (docs.length == 0) {
