                ...
                if (err == errCodes.GAME_NOT_FOUND) {
                    ...
                    db.collection("games").findAndModify({ _id: game._id, status: 'InProgress' }, {}, { $addToSet: { scores: newScore } }, {new:true}, function (err, updatedGameObj) {
                        if (err) {
                            ...
                            // if so, mark the game as Completed, declare winner, and update rankings.
                                ...
                                for (var i = 0; i < updatedGameObj.scores.length; i++) {
                                    ...
                                    if (radiantWinReports >= 5) {
                                        ReportScoreInternal(db, game._id, 'Radiant', function (err, updatedGameObj) {
                                            UpdateRatings(db, updatedGameObj, 'Radiant', function (err) {
                                                return callback(err, updatedGameObj);
                                            });
                                        });
                                    } else if (direWinReports >= 5) {
                                        ReportScoreInternal(db, game._id, 'Dire', function (err, updatedGameObj) {
                                            UpdateRatings(db, updatedGameObj, 'Dire', function (err) {
                                                return callback(err, updatedGameObj);
                                            });
                                        });
                                    }
...
function UpdateRatings(db, gameObject, winner, callback) {
    var playersLeftToUpdate = 10;
    for (var i = 0; i < gameObject.players.length; i++) {
        var player = gameObject.players[i];

        db.collection("players").findOne({_id: player.id}, function (err, oldPlayerRecord) {
            if (err) {
                return callback(errCodes.DATABASE_FAILURE);
            } else {
                var newRating;
                if (player.team == winner) {
                    newRating = Math.max(0, oldPlayerRecord.rating + 25);
                } else {
                    newRating = Math.max(0, oldPlayerRecord.rating - 25);
                }

                db.collection("players").findAndModify({ _id: player.id }, {}, { $set: { rating: newRating } }, {new:true}, function (err, updatedPlayerObj) {
                    assert.equal(err, null, "UpdateRatings: database failure");
                    assert.equal(updatedPlayerObj.rating, newRating, "UpdateRatings: expected player rating " + newRating + ", actual " + updatedPlayerObj.rating);
                    playersLeftToUpdate--;
                    if (playersLeftToUpdate == 0) {
...
/**
 * Takes in an array of format: [{id: ObjectId, team = 'Radiant'}, {id: ObjectId, team = 'Dire'}, etc]
 * Returns an object of format: {Dire: [PlayerObj1, PlayerObj2, etc], Radiant: [PlayerObj6, PlayerObj7, etc]}
 */
DbClient.prototype.ResolvePlayerNames = function (db, players, callback) {
    var output = {Dire: [], Radiant: []};
    var playersLeftToResolve = players.length;
    players.forEach(function (value, index, players) {
        db.collection("players").findOne({_id: value.id}, function (err, user) {
            if (err) {
                return callback(errCodes.DATABASE_FAILURE);
            } else {
                playersLeftToResolve--;
                switch (value.team) {
                    case "Dire":
                        output.Dire.push(user);
                        break;
                    case "Radiant":
                        output.Radiant.push(user);
                        break;
                }
                if (playersLeftToResolve == 0) {
                    return callback(errCodes.SUCCESS, output);
                }
            }
        });
    });
                ...
                if (err) {
                    ...
                    if (!game.gameCreator.equals(user._id)) {
                        ...
                        if (game.players.length < 0 || game.players.length > 10) {
                            ...
                            return callback(errCodes.GAME_NOT_READY, game);
                        } else {
                                ...
                                if (err) {
                                    ...
                                    GetPlayersForGame(db, game.players, function (err, players) {
                                        ranking.BalancedShuffle(players, function (playerSplit) {
                                            ...
                                            db.collection("games").findAndModify({_id: game._id}, {}, { $set : {players: playersWithTeams}}, {new:true}, function (err, startedGame) {
                                                if (err) {
                                                    return callback(errCodes.DATABASE_FAILURE);
                                                } else {
                                                    return callback(errCodes.SUCCESS, startedGame);
                                                }
                                            });
                                        });
                                    });
...
function FindUser(db, playerSteam64Id, callback) {
    db.collection("players").findOne({ steam64Id: playerSteam64Id }, function (err, user) {
        //trace.debug("FindUser " + playerSteam64Id + " : err code " + err);
        if (err) {
...
function FindActiveGame(db, gameNum, callback) {
    FindLatestSeason(db, function(err, latestSeason) {
        if (err == errCodes.NO_SEASONS_FOUND) {
            return callback(err.NO_SEASONS_FOUND);
        }
...
/*
 * Description: adds a player to a game, returns updated game.
*/ 
            ...
            FindActiveGame(db, gameNum, function (err, game) {
                if (err) {
                    ...
                    FindLatestSeason(db, function (err, latestSeason) {
                        ...
                        // add user to game
                        db.collection("games").findAndModify({seasonId: latestSeason._id, 'gameNum': gameNum, status: { $in: ['WaitingForPlayers', 'InProgress'] } }, {}, { $addToSet: { players: {id: user._id, team: 'ToBeDecided'} } }, {new:true}, function (err, doc) {
                            if (err) {
                                return callback(errCodes.DATABASE_FAILURE);
                            } else {
                                return callback(errCodes.SUCCESS, doc);
                            }
                        });
                    });
                }
