...
function GetPlayersForGame(db, players, callback) {
    var temp = db.collection("players").find({ _id: { $in: players } }).toArray(function (err, docs) {
        if (err) {
            return callback(errCodes.DATABASE_FAILURE);
        } else {
            var results = docs;
...
*/ 
DbClient.prototype.Register = function (db, playerSteam64Id, username, initialRating, callback) {
    ...
    FindUser(db, playerSteam64Id, function (err, doc) {
        if (err == errCodes.USER_NOT_REGISTERED) {
            ...
            db.collection("players").insert({ steam64Id : playerSteam64Id, 'username': username, rating : initialRating }, { w: 1 }, function (err, docs) {
                if (err) {
                    return callback(errCodes.DATABASE_FAILURE);
                } else {
                    return callback(errCodes.SUCCESS);
