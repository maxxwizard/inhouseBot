...
module.exports.TestAllFunctionality = function (DbClient) {
                ...
                seasonColl.save({ _id: randSeasonId, seasonNum: 1, startDate: Date(2014, 9, 1), endDate: Date(2014, 12, 31), name: 'Season 1' }, function (err, docs) {
                    ...
                    db.collection("players").insert({ _id: randPlayerId, username: 'maxxwizard', rating: 1500, steam64Id: '76561197968837492', admin: 1 }, function (err, docs) {
                        ...
                        db.collection("players").insert({ _id: randPlayerId2, username: 'saltydog', rating: 1400, steam64Id: '76561198029982751' }, function (err, docs) {
                            ...
                            TestGetCurrentGamesFunctionality(function () {                                  // show zero game scenario
                                TestNewGameFunctionality("76561197968837492", function () {                 // add game #1
                                    TestGetCurrentGamesFunctionality(function () {                          // show 1 game scenario
                                        TestNewGameFunctionality("76561198029982751", function () {         // add game #2
                                            TestGetCurrentGamesFunctionality(function () {                  // show 2 game scenario
                                                TestNewGameFunctionality("76561198029982751", function () { // add 3rd game with duplicate steam64id (should produce failure)
                                                    TestCancelGameFunctionality('76561197968837492', 0);    // cancel game that doesn't exist
                                                    TestCancelGameFunctionality('76561198029982751', 1, function (err) { // cancel game as unauthorized user
                                                        TestCancelGameFunctionality('1234567890', 1, function (err) // cancel game with an unregistered user
                                                        {
                                                            TestCancelGameFunctionality('76561197968837492', 1); // cancel game as admin
                                                        }); 
                                                    });    
                                                    TestCancelGameFunctionality('76561198029982751', 2);    // cancel game as game creator
                                                });
                                            });
                                        });
                                    });
    ...
    function TestNewGameFunctionality(steam64id, callback) {
        DbClient.NewGame(steam64id, function (err, newGame) {
            if (err) {
                trace.warn("TestNewGame: game was not created with error code " + err);
                if (err == errCodes.USER_ALREADY_SIGNED) {
                    trace.warn("TestNewGame: user already signed into game #" + newGame.gameNum);
                }
            } else {
                trace.log("TestNewGame: new game #" + newGame.gameNum + " created");
            }
            if (callback) {
                callback();
            }
