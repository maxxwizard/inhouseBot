...
module.exports.TestAllFunctionality = function (DbClient) {
    ...
    DbClient.Connect(function (err, db) {
        ...
        if (config.runDatabaseTests) {
            db.dropDatabase(function (err, result) {
                trace.debug("BotTest: database dropped in prep for database test!");
                RunDatabaseTests(db, function () {
                    if (config.runHandlerTests) {
                        db.dropDatabase(function (err, result) {
                            trace.debug("BotTest: database dropped in prep for handler test!");
                            RunHandlerTests(DbClient, db, function () {
                                // test finished
                                //db.close();
                            });
                        });
    ...
    function RunDatabaseTests(db, callback) {
                    ...
                    TestSignUnsignSuite(db, function () {
                        TestStartGameFunctionality(db, function () {
                            TestReportScoreFunctionality(db, function () {
                                trace.debug("DATABASE TESTS FINISHED!");
                                if (callback)
                                    callback();
                            });
                        });
    ...
    function RunHandlerTests(DbClient, db, callback) {
        // test !register
        handlers.universalMessageHandler(DbClient, db, "76561197968837492", "maxxwizard", "!register", function (err, responseMsg) {
            assert.equal(err, errCodes.SUCCESS, "user already registered scenario : " + err + " : " + responseMsg);
            handlers.universalMessageHandler(DbClient, db, "76561197968837492", "maxxwizard", "!register", function (err, responseMsg) {
                ...
                handlers.universalMessageHandler(DbClient, db, "76561197968837492", "maxxwizard", "!games", function (err, responseMsg) {
                    ...
                    handlers.universalMessageHandler(DbClient, db, "76561197968837492", "maxxwizard", "!newSeason 'Season 1'", function (err, responseMsg) {
                        ...
                        handlers.universalMessageHandler(DbClient, db, "76561197968837492", "maxxwizard", "!reportWin", function (err, responseMsg) {
                            ...
                            // TODO: write full suite of test for handler
                    ...
                    for (var i = 1; i <= 5; i++) {
                        DbClient.ReportScore(db, i, "win", "Radiant", function (err, updatedGameObj) {
                            ...
                            if (numReportsLeft == 0) {
                                ...
                                trace.log("TestReportScore: all test passed!");
                                return callback();
                            } else {
    ...
    function TestNewSeasonFunctionality(db, callback) {
        DbClient.NewSeason(db, '76561197968837492', "Season 1", function (err) {
            ...
            DbClient.NewSeason(db, '76561197968837492', "Season 2", function(err, newSeason) {
                ...
                trace.log("TestNewSeason: all test passed!");
                if (callback) {
                    callback();
                }
            });
    ...
    function TestStartGameFunctionality(db, callback) {
        ...
        SeedTestPlayers(db, 10, function () {
            ...
            CreateGameAndSignXPlayers(db, 10, function(err, newGameObj) {
                // we've finished signing all 10 so now we can run our StartGame test
                    ...
                    DbClient.StartGame(db, '123456789', newGameNum, function (err) {
                        ...
                        DbClient.StartGame(db, '76561197968837492', 3, function (err) {
                            ...
                            DbClient.StartGame(db, 2, newGameNum, function (err) {
                                ...
                                DbClient.StartGame(db, 1, newGameNum, function (err, startedGame) {
                                    assert.equal(err, errCodes.SUCCESS, "TestStartGame: 10-player scenario failed with error code " + err);

                                    DbClient.ResolvePlayerNames(db, startedGame.players, function (err, resolvedPlayers) {
                                        //trace.debug(JSON.stringify(resolvedPlayers));
                                        trace.log("TestStartGame: all test passed!");
                                        ...
                                        DbClient.CancelGame(db, '1234567890', 1, false, function (err) {
                                            ...
                                            DbClient.CancelGame(db, '76561197968837492', 1, false, function (err) {
                                                ...
                                                DbClient.CancelGame(db, '76561198029982751', 2, false, function (err) {
                                                    ...
                                                    // execute callback
                                                    trace.log("TestNewGameCancelGameSuite: all test passed!");
                                                    if (callback) {
                                                        callback();
                                                    }
                                                });
                                    ...
                                    DbClient.UnsignGame(db, "123456789", newGame1.gameNum, function (err) {
                                        ...
                                        DbClient.UnsignGame(db, "76561197968837492", newGame1.gameNum, function (err) {
                                            ...
                                            DbClient.UnsignGame(db, "76561197968837492", newGame1.gameNum, function (err) {
                                                ...
                                                // execute callback
                                                trace.log("TestSignUnsignSuite: all test passed!");
                                                if (callback) {
                                                    callback();
                                                }
                                            });
            ...
            DbClient.Register(db, '76561198029982751', 'saltydog', 1400, 0, function (err) {
                ...
                DbClient.Register(db, "76561197968837492", "maxxwizard", ranking.InitialRating, 0, function (err) {
                    ...
                    DbClient.Register(db, "76561198051766196", "gemini", ranking.InitialRating, 0, function (err) {
                        ...
                        trace.log("TestRegisterSuite: all test passed!");
                        if (callback) {
                            callback(err);
                        }
                    });
