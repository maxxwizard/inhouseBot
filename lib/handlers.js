    ...
    if (message.substring(0, 1) == "!" && message.length >= 5) {
        ...
        switch (trueCommand) {
                    ...
                    DbClient.SignGame(db, playerSteam64Id, targetGameNum, function (err, updatedGame) {
                        switch (err) {
                            ...
                            case errCodes.GAME_NOT_FOUND:
                                ...
                                break;
                            case errCodes.USER_ALREADY_SIGNED:
                                responseMsg = sprintf("%s, you are already signed into game #%u .", personaName, targetGameNum);
                                break;
                            case errCodes.SUCCESS:
            ...
            case "!start":
                var targetGameNum = parseInt(param);
                if (targetGameNum >= 0) {
                    DbClient.SignGame(db, playerSteam64Id, targetGameNum, function (err, startedGame) {
                        switch (err) {
                            case errCodes.DATABASE_FAILURE:
                ...
                DbClient.NewSeason(db, playerSteam64Id, param, function (err, newSeason) {
                    switch (err) {
                        case errCodes.DATABASE_FAILURE:
                            ...
                            break;
                        case errCodes.USER_NOT_REGISTERED:
                            responseMsg = sprintf("%s, you must register before you can create a new season.");
                            break;
                        case errCodes.UNAUTHORIZED:
                            responseMsg = sprintf("%s, you must be an admin to create a new season.");
                            break;
                        case errCodes.SUCCESS:
